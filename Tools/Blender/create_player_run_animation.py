"""Create only Run on the approved rig through Blender MCP; no rig/mesh edits.
24 intervals at 30 fps, keys 1..25, preview playback 1..24 (no duplicate endpoint).
Adapted from the approved Jog generator; loads Jog without editing its curves.
...
action=bpy.data.actions.new('Run'); action.use_fake_user=True
rig.animation_data.action=action
action['description']='In-place middle-tier football run; 24 intervals at 30 fps; duplicate closing key at 25.'
rig['pose_usage']='Run locomotion plus preserved static Pose_* inspection actions'
...
for frame in range(1,26):
 t=(frame-1)/24; phase=2*math.pi*t
 for p in rig.pose.bones:
  p.rotation_mode='QUATERNION'; p.rotation_quaternion=(1,0,0,0); p.location=(0,0,0); p.scale=(1,1,1)
 rotate('Hips',(0,1,0),3*math.cos(phase))
 rotate('Spine',(1,0,0),-9)
 rotate('Chest',(0,1,0),-6*math.cos(phase))
 rotate('Head',(0,1,0),3*math.cos(phase))
 rotate('Head',(1,0,0),9)
...
for i in range(24):
 for sub in (0.125,0.25,0.375,0.5,0.625,0.75,0.875):
  ...
  clearance[(i+1)%24]=max(clearance[(i+1)%24],lift)
# Enforce exact half-cycle equality despite floating-point grounding noise.
for i in range(12): clearance[i]=clearance[i+12]=max(clearance[i],clearance[i+12])
fc=next(f for f in action.fcurves if f.data_path=='pose.bones["Hips"].location' and f.array_index==1)
for k in fc.keyframe_points: k.co.y+=clearance[(int(k.co.x)-1)%24]
for sample in samples:
 lift=clearance[(sample['frame']-1)%24]
...
start=snapshot(1); end=snapshot(25)
error=max((a-b).length for a,b in zip(start,end))
assert error<1e-5
assert set(bpy.data.actions.keys())==prior|{'Run'}
assert 'Jog' in bpy.data.actions and 'Sprint' not in bpy.data.actions
for f in range(1,26):
 scene.frame_set(f)
...
scene.frame_end=25
bpy.ops.export_scene.gltf(filepath=str(OUT/'lowpoly_human_run_validation.glb'),export_format='GLB',use_selection=True,export_yup=False,export_animations=True,export_nla_strips=False,export_frame_range=True,export_force_sampling=True,export_skins=True,export_current_frame=False)
# Blender 3.3 merges rigid equipment channels into an action named Animation.
# Keep those channels in the single Run clip and normalize its metadata name.
glb=OUT/'lowpoly_human_run_validation.glb'; raw=glb.read_bytes()
json_size=struct.unpack_from('<I',raw,12)[0]; doc=json.loads(raw[20:20+json_size])
assert len(doc['animations'])==1
doc['animations'][0]['name']='Run'
...
scene.render.ffmpeg.constant_rate_factor='HIGH'; scene.render.filepath=str(PRE/'Run_loop.mp4')
bpy.ops.render.render(animation=True)
(PRE/'validation.json').write_text(json.dumps({'source':SOURCE.name,'source_sha256':source_hash,'action':'Run','fps':30,'keyed_range':[1,25],'playback_range':[1,24],'duration_seconds':24/30,'endpoint_mesh_error_m':error,'root_translation':False,'rig_weight_corrections':None,'samples':samples},indent=2))
print('RUN_COMPLETE endpoint_error=',error)
